...
namespace Tenweb_Manager {
    ...
    class Helper
    {
        ...
        private static function site_state($force_send = false, $current_blog_id = null)
        {
            ...
            $site_current_state = md5(json_encode(self::get_site_info($current_blog_id)));
            ...
            if ($site_hash !== $site_current_state || $transient == false) {
                ...
                $state_data['site_info'] = self::get_site_info($current_blog_id);
        ...
        public static function get_tmp_dir($force_upload_dir = false)
        {
            ...
            if (!$force_upload_dir && !TENWEB_MIGRATION_DEBUG && is_dir($sys_tmp_dir) && is_readable($sys_tmp_dir)) {
        ...
        public static function define_configs()
        {
            if (file_exists(TENWEB_DIR . '/config.json')) {
                $config_json = file_get_contents(TENWEB_DIR . '/config.json');
                $config_json = json_decode($config_json, true);
            }
            $TENWEB_MIGRATION_DEBUG = isset($config_json['TENWEB_MIGRATION_DEBUG']) ? $config_json['TENWEB_MIGRATION_DEBUG'] : 0;
            $TENWEB_MIGRATION_ARCHIVE_TYPE = isset($config_json['TENWEB_MIGRATION_ARCHIVE_TYPE']) ? $config_json['TENWEB_MIGRATION_ARCHIVE_TYPE'] : "gzip";
            $TENWEB_MIGRATION_MAX_FILES_RESTART = isset($config_json['TENWEB_MIGRATION_MAX_FILES_RESTART']) ? $config_json['TENWEB_MIGRATION_MAX_FILES_RESTART'] : 20000;
            $TENWEB_MIGRATION_BULK_FILES_COUNT = isset($config_json['TENWEB_MIGRATION_BULK_FILES_COUNT']) ? $config_json['TENWEB_MIGRATION_BULK_FILES_COUNT'] : 10000;
            $TENWEB_MIGRATION_BULK_DB_ROWS_COUNT = isset($config_json['TENWEB_MIGRATION_BULK_DB_ROWS_COUNT']) ? $config_json['TENWEB_MIGRATION_BULK_DB_ROWS_COUNT'] : 500;
            $TENWEB_MIGRATION_EXEC_TIME_OFFSET = isset($config_json['TENWEB_MIGRATION_EXEC_TIME_OFFSET']) ? $config_json['TENWEB_MIGRATION_EXEC_TIME_OFFSET'] : 10;

            define('TENWEB_MIGRATION_DEBUG', $TENWEB_MIGRATION_DEBUG);
            define('TENWEB_MIGRATION_ARCHIVE_TYPE', $TENWEB_MIGRATION_ARCHIVE_TYPE);
            define('TENWEB_MIGRATION_MAX_FILES_RESTART', $TENWEB_MIGRATION_MAX_FILES_RESTART);
            define('TENWEB_MIGRATION_BULK_FILES_COUNT', $TENWEB_MIGRATION_BULK_FILES_COUNT);
            define('TENWEB_MIGRATION_BULK_DB_ROWS_COUNT', $TENWEB_MIGRATION_BULK_DB_ROWS_COUNT);
            define('TENWEB_MIGRATION_EXEC_TIME_OFFSET', $TENWEB_MIGRATION_EXEC_TIME_OFFSET);

            define('TENWEB_MIGRATION_DEBUG_DEFAULT', 0);
            define('TENWEB_MIGRATION_ARCHIVE_TYPE_DEFAULT', 'gzip');
            define('TENWEB_MIGRATION_MAX_FILES_RESTART_DEFAULT', 20000);
            define('TENWEB_MIGRATION_BULK_FILES_COUNT_DEFAULT', 10000);
            define('TENWEB_MIGRATION_BULK_DB_ROWS_COUNT_DEFAULT', 500);
            define('TENWEB_MIGRATION_EXEC_TIME_OFFSET_DEFAULT', 10);
