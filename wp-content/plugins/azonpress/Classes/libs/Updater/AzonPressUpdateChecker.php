<?php
namespace Azonpress\Classes\Libs\Updater;

class AzonPressUpdateChecker
{
    /**
        ...
        if ('valid' != $licenseStatus) {
            ...
            if ($key) {
                ...
                if ($licenseData && $licenseData->license == 'expired') {
                    if (isset($_GET['page']) && $_GET['page'] == 'azonpress' || current_user_can('manage_options')) {
                        $renewUrl = admin_url('admin.php?page=azonpress#/settings/licensing');
                        add_action('admin_notices', function () use ($renewUrl, $licenseData) {
                            echo '<div class="error error_notice' . $this->get_var('option_group') . '"><p>' .
                                'Your AzonPress license <b>expired at ' . date('d M Y', strtotime($licenseData->expires)) . '</b>, Please ' .
                                '<a href="' . $renewUrl . '"><b>Click Here to Renew Your License</b></a>'
                                .
                                '</p></div>';
                        });
                    }
                    ...
                    add_action('admin_notices', function () {
                        echo '<div class="error error_notice' . $this->get_var('option_group') . '"><p>' .
                            sprintf(__('The %s license needs to be activated. %sActivate Now%s', 'azonpress'),
    ...
    {
        if (function_exists('azonpress_table_admin_role') && !current_user_can(azonpress_table_admin_role())) {
            wp_send_json_error(array(
