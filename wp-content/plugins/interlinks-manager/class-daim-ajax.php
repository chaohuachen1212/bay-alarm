...
class Daim_Ajax{
    ...
    private function __construct() {
	    ...
	    add_action( 'wp_ajax_update_http_status_archive', array( $this, 'update_http_status_archive' ) );
    ...
    public function update_interlinks_archive(){
        ...
        foreach ($posts_a as $key => $single_post) {
	        ...
	        $safe_sql     = $wpdb->prepare("SELECT iil FROM $table_name WHERE url = %s ", $permalink);
	        $juice_obj = $wpdb->get_row($safe_sql);
        ...
        foreach($archive_a as $key => $single_archive){
            ...
            $query_groups[$query_index][] = $wpdb->prepare("( %d, %s, %s, %s, %d, %d, %d, %d, %d, %d )",
         ...
         * executed
         */
        $query_start = "INSERT INTO $table_name (post_id, post_title, post_type, post_date, manual_interlinks, auto_interlinks, iil, content_length, recommended_interlinks, optimization) VALUES ";
        $query_end = '';
	...
	public function update_http_status_archive(){
		...
		//check the capability
		if(!current_user_can(get_option( $this->shared->get('slug') . "_http_status_menu_required_capability"))){echo "Invalid Capability"; die();}

		//Clear the existing cron event
		wp_clear_scheduled_hook( 'daextdaim_cron_hook' );

		//Create a new list of URLs to check
		$this->shared->create_http_status_list();
    ...
    public function track_internal_link(){
        ...
        //Sanitization
	    $link_type = isset($_POST['link_type']) and sanitize_key($_POST['link_type']) === 'ail' ? $link_type = 0 : $link_type = 1;
        $source_post_id = isset($_POST['source_post_id']) ? intval($_POST['source_post_id'], 10) : null;
        $target_url = isset($_POST['target_url']) ? mb_substr(esc_url_raw($_POST['target_url']),0, 2038) : null;

        //Validation
	    if(is_null($source_post_id) or is_null($target_url)){
	    	return 'invalid-data';
	    	die();
	    }
    ...
    public function generate_interlinks_suggestions(){
        ...
        if( !isset($posts_ranking_a) or count($posts_ranking_a) <= 5){
            ...
            $output = '<p>' . esc_html__('There are no interlinks suggestions at the moment, please use this functionality when you have at least five posts (other than the current one) that match the criteria you defined in the "Suggestions" options.', 'daim') . '</p>';
        ...
        //Store the HTML of the clipboard icon
        $clipboard_icon_svg = '<?xml version="1.0" encoding="utf-8"?>
        ...
        for($i=1;$i<=5;$i++){
            ...
            $output .= '<div class="daim-interlinks-suggestions-item">';
            $output .= '<div class="daim-interlinks-suggestions-container-left">';
            $output .= '<a class="daim-interlinks-suggestions-link" data-index="' . esc_attr($i) .'" href="' . esc_url( get_permalink($random_id)) . '">' . esc_html( get_the_title($random_id) ) . '</a>';
	        $output .= '<div class="daim-interlinks-suggestions-post-type">' . esc_html( $post_type_object->labels->singular_name ) . '</div>';
	        $output .= '</div>';
	        $output .= '<div class="daim-interlinks-suggestions-copy-button" data-index="' . esc_attr($i) .'">' . $clipboard_icon_svg . '</div>';
            $output .= '</div>';
