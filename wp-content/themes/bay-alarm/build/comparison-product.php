<?php
  // Template Name: Comparison Product
  update_option('current_page_template', 'comparison-product');
  get_header();
...
<section class="comparison-product--content">
  <div class="container">
    <div class="content-nav-wrap">
      <h2><?php the_field('tables_nav_buttons_title'); ?></h2>
      <div class="nav-wrap">
        <a href="#bay-alarm-medical" class="is-active">Overview</a>
        <?php
          $c = 1;
           $nav_arr = [];
          if( have_rows('tables_list') ):
          while( have_rows('tables_list') ): the_row();
          $text = strtolower(str_replace(' ', '-', get_sub_field('nav_name')));
          array_push($nav_arr, $text);
          if($c > 1):
        ?>
        <a href="#<?php echo $text; ?>"><?php the_sub_field('nav_name'); ?></a>
        <?php
          endif; $c++; endwhile; endif;
        ?>
       
      </div>
    </div>

    <div class="table-secs-wrap">
      <div class="table-sec">
        ...
        <?php
          ...
          while( have_rows('tables_list') ): the_row();
        ?>
        <div class="table" id="<?php echo $nav_arr[$n]; ?>">
          <h2><?php the_sub_field('title'); ?></h2>
          <div class="top-row">
            <div class="col">
              <img src="<?php the_sub_field('bam_icon'); ?>" alt="Icon">
          ...
          <div class="rows-wrap">
            ...
            <div class="row">
              ...
              <div class="col">
                <?php if (get_sub_field('bam_turn_on_mark')): ?>
                <div class="check-wrap">
                  <?php if (get_sub_field('bam_option_pick')==='yes'): ?>
                  <?php include 'img/comparison/check.svg'; ?>
                  <p>Yes</p>
                  <?php else: ?>
                    <?php include 'img/comparison/close.svg'; ?>
                    <p>No</p>
                  <?php endif; ?>
                </div>
                <?php else: ?>
                  <p><?php the_sub_field('bam_text'); ?></p>
                <?php endif; ?>
              </div>
              <div class="col">
                <?php if (get_sub_field('other_turn_on_mark')): ?>
                <div class="check-wrap">
                  <?php if (get_sub_field('other_option_pick')==='yes'): ?>
                  <?php include 'img/comparison/check.svg'; ?>
                  <p>Yes</p>
                  <?php else: ?>
                    <?php include 'img/comparison/close.svg'; ?>
                ...
                <?php else: ?>
                  <p><?php the_sub_field('other_text'); ?></p>
                <?php endif; ?>
              </div>
            </div>
            <?php
